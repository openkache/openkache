...
function request_wire_step_bound(
  ...
  for (const step of steps) {
    switch (step.kind) {
      ...
      case "value_length_field": add(contract.v1.max_varuint_bytes); add(contract.max_value_bytes); break
      case "conditional": add(request_wire_step_bound(contract, step.steps)); break
      case "constant": add(step.bytes.length); break
      case "trailing_field": add(contract.v1.max_varuint_bytes); add(contract.max_value_bytes); break
    }
  }
  return bound
}
