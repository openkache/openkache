...
export function request_wire_plan(
  ...
  const symbolic = (field: Wire_Operation_Field_Plan, value: string, location: string): void => {
  ...
  const parse = (values: readonly unknown[], location: string, depth = 0): readonly Wire_Request_Step[] => {
      ...
      switch (kind) {
        case "fixedField": {
          ...
          if (field.encoded_width !== undefined && field.encoded_width !== bytes) {
            throw new Error(`${step_location}.${kind}.bytes must match ${name}'s encoded width ${field.encoded_width}`)
          }
          return { kind: "fixed_field", field: field.index, bytes }
        }
        case "packed": {
          const raw_fields = array_member(value, "fields", `${step_location}.${kind}`)
          if (raw_fields.length === 0) throw new Error(`${step_location}.${kind}.fields must not be empty`)
          let occupied = 0
          const packed_fields = raw_fields.map((raw_field, field_index) => {
            ...
            if ((occupied & mask) !== 0) throw new Error(`${field_location}.mask overlaps another packed field`)
            occupied |= mask
            const raw_values = array_member(packed, "values", field_location)
            if (raw_values.length === 0) throw new Error(`${field_location}.values must not be empty`)
            ...
            const values = raw_values.map((raw_mapping, mapping_index) => {
              ...
              if ((bits & ~mask) !== 0 || seen_values.has(value_name) || seen_bits.has(bits)) {
                throw new Error(`${mapping_location} contains an invalid or duplicate packed mapping`)
              }
          ...
          const reserved_mask = optional_integer_member(value, "reservedMask", `${step_location}.${kind}`, 0, 0xff) ?? 0
          const constant_bits = optional_integer_member(value, "constantBits", `${step_location}.${kind}`, 0, 0xff) ?? 0
          if ((reserved_mask & occupied) !== 0 || (constant_bits & occupied) !== 0 || (reserved_mask & constant_bits) !== 0) {
            throw new Error(`${step_location}.${kind} reserved/constant bits overlap packed fields`)
          }
        ...
        case "valueLengthField": {
          ...
          if (length !== "varuint") throw new Error(`${step_location}.${kind}.length must be varuint`)
        ...
        case "conditional": {
          ...
          if (nested.length === 0) throw new Error(`${step_location}.${kind}.steps must not be empty`)
          return { kind: "conditional", field: field.index, equals, steps: parse(nested, `${step_location}.${kind}.steps`, depth + 1) }
        }
        case "constant": {
          const hex = string_member(value, "hex", `${step_location}.${kind}`)
          if (!/^(?:[0-9a-f]{2})+$/.test(hex)) throw new Error(`${step_location}.${kind}.hex must be non-empty lowercase hexadecimal bytes`)
          return { kind: "constant", bytes: Array.from({ length: hex.length / 2 }, (_, byte) => Number.parseInt(hex.slice(byte * 2, byte * 2 + 2), 16)) }
        }
        case "trailingField": {
          const name = string_member(value, "field", `${step_location}.${kind}`)
          const field = resolve_field(name, `${step_location}.${kind}.field`)
