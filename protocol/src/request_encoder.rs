...
enum FramePiece {
    Static(&'static [u8]),
    Encoded {
        bytes: [u8; crate::MAX_VARUINT_BYTES],
        len: u8,
    },
    Field(usize),
}
...
impl EncodePlan {
        ...
        for step in steps {
            ...
            match *step {
                ...
                RequestFrameStep::VarUIntField { field } => {
                    let value = self.use_field(fields, field)?;
                    let (encoded, len) = crate::encode_varuint(canonical_u64(value)?);
                ...
                | RequestFrameStep::ValueLengthPrefixField { field } => {
                    ...
                    let (encoded, len) = crate::encode_varuint(value.len() as u64);
                    self.push_encoded(&encoded[..len])?;
    ...
    fn push_encoded(&mut self, value: &[u8]) -> Result<()> {
        let mut bytes = [0; crate::MAX_VARUINT_BYTES];
...
fn validate_value_length(value_len: usize) -> Result<()> {
    if value_len > crate::MAX_VALUE_BYTES {
        return Err(ProtocolError::ValueTooLarge {
            size: value_len,
