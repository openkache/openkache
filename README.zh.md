...
**OpenKache 为什么快?因为一个请求从不离开它最初落到的核心。**

大多数服务器把工作分散到在核心之间游走的线程池上。这个代价并不便宜。每当请求跨核心跳转,你都要
为它付费:锁竞争、互斥量、上下文切换,以及在缓存之间来回弹跳的缓存行,每一样都强制一次同步和一次
拷贝。负载之下,决定吞吐量上限的是这种协调开销,而非实际工作。

OpenKache 拒绝付费。下面的每个决策都删除了一整类这样的成本:

| 设计决策 | 它删除的成本 | 依据 |
|---|---|---|
| **Thread-per-core, shared-nothing(无共享)** | 锁竞争、上下文切换、跨核心缓存行弹跳 | [KVell (SOSP '19)](https://dl.acm.org/doi/10.1145/3341301.3359628) |
| **段组顺序写入** | 随机写 IOPS、闪存写放大 | [FairyWren (OSDI '24)](https://www.usenix.org/conference/osdi24/presentation/mcallister) |
| **`io_uring` + 直接 I/O** | 系统调用开销、内核页缓存拷贝 | — |
| **用 Rust 编写** | GC 停顿;由编译器而非生产环境捕获的数据竞争 | — |

KVell 的教训很直白:在现代 SSD 上,瓶颈不再是磁盘,而是 CPU。因此 OpenKache 把每个工作线程
固定到自己的核心,交给它自己的数据,让它在没有任何锁的情况下运行。网络工作线程与存储工作线程
各自独占一个核心,并通过一条无锁 SPSC 队列对话,因此解析绝不会阻塞磁盘 I/O,磁盘 I/O 也绝不会
阻塞解析。

FairyWren 点出第二点:闪存缓存把寿命耗在小的随机写上。因此 OpenKache 从不发出这样的写。它把
许多键合并成一次顺序的 **段组(segment-group)** 刷写,像一列满载的地铁而非每位乘客一辆车,把
SSD 的顺序带宽用到极限。

**正因为用 Rust 编写,才敢把性能推到极限而依然安全:贴近裸机的控制,没有垃圾回收器来拖慢快
路径,数据竞争由编译器而非生产环境来捕获。**
