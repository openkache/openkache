...
// ItemID is the opaque variable-length identifier carried by the wire protocol.
type ItemID []byte

// NewItemID copies a wire item ID containing zero through SmithyItemIDBytes bytes.
func NewItemID(value []byte) (ItemID, error) {
	...
	return append(ItemID(nil), value...), nil
}

// Bytes returns a copy of the item-ID bytes.
...
type nativeClient interface {
	execute(context.Context, uint32, uint32, []byte, []byte, SetOptions) (nativeResult, error)
...
func (c *Client) invoke(
	...
	return c.invokeTyped(ctx, operation, SmithyFFIKeySpecBytes, key, value, options)
}

func (c *Client) invokeTyped(
	ctx context.Context,
	operation uint32,
	keySpec uint32,
	key, value []byte,
	...
	return c.invokeNative(ctx, func(native nativeClient) (nativeResult, error) {
		return native.execute(ctx, operation, keySpec, key, value, options)
	})
}
