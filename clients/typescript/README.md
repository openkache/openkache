...
Keys follow the configured `key_type` (`text` by default): text keys are exact
...
- `key_type` selects `text` (the default), `bytes`, or `integer` for formatted
...
contract for opaque item IDs of up to 32 bytes and opaque protocol values.
