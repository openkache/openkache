...
| Value model | [`value/`](value/) | Owned `Value` algebra and bounded structured payload codec |
...
not interpret it. The TypeScript mapped `set` / `get` facade now uses the
common structured value model and native JavaScript mapping. Explicit
...
| Structured values | Common cross-language value model | Mapped `set`/`get` use the maintained structured profile; JSON helpers remain explicit | Implemented |
