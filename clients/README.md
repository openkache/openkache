...
| Rust | [`rust/`](rust/) | Protocol v1 end-user SDK; byte APIs use v1 OpaqueBytes serialization |
| CLI | [`cli/`](cli/) | Bash-friendly one-shot and interactive client binary |
| TypeScript / JavaScript | [`typescript/`](typescript/) | Protocol v1 Node-API SDK with typed key conversion, OpaqueBytes, and shared canonical-JSON operations; legacy metadata helpers remain package-local |
...
`set_json` / `get_json` methods for cross-language values. Formatted clients
use the normative `OpaqueBytes` payload name, while exact-item-ID raw clients
