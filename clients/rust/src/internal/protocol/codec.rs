...
//! [`CodecError`](crate::internal_protocol::codec::CodecError) to their local error boundary.

use crate::internal_protocol::{decode_varuint, encode_varuint};
...
pub fn validate_kind(
    ...
    if payload.len() > crate::internal_protocol::MAX_VALUE_BYTES {
...
pub fn encode_union(tag: u8, payload: &[u8], allowed_tags: &[u8]) -> Result<Vec<u8>, CodecError> {
    ...
    let mut output = Vec::with_capacity(
        1usize
            .saturating_add(crate::internal_protocol::MAX_VARUINT_BYTES)
            .saturating_add(payload.len())
            .min(crate::internal_protocol::MAX_VALUE_BYTES),
    );
    output.push(tag);
...
impl<'a> UnionView<'a> {
    ...
    pub fn new(encoded: &'a [u8], allowed_tags: &[u8]) -> Result<Self, CodecError> {
        if encoded.len() > crate::internal_protocol::MAX_VALUE_BYTES {
            return Err(VALUE_TOO_LARGE);
        }
...
fn append_length_delimited(output: &mut Vec<u8>, value: &[u8]) -> Result<(), CodecError> {
    ...
    if next_len > crate::internal_protocol::MAX_VALUE_BYTES {
        return Err(VALUE_TOO_LARGE);
    }
...
fn read_length_delimited(
    ...
    let view = crate::internal_protocol::compact::read_length_delimited(payload, cursor)
        .map_err(|_| error)?
        .ok_or(error)?;
