...
use crate::internal_protocol::request::{
    MAX_REQUEST_FRAME_STATE_SLOTS, RequestFrameLayout, RequestFrameStep,
};
use crate::internal_protocol::segments::RequestPrefix;
...
pub fn encode_request_frame_with_id(
    ...
    let (request_id_bytes, request_id_len) = crate::internal_protocol::encode_varuint(request_id);
...
enum FramePiece {
    Static(&'static [u8]),
    Encoded {
        bytes: [u8; crate::internal_protocol::MAX_VARUINT_BYTES],
        len: u8,
    },
    Field(usize),
}
...
impl EncodePlan {
    fn new(field_count: usize, request_id: u64) -> Self {
        let request_id_len = crate::internal_protocol::encode_varuint(request_id).1;
        Self {
        ...
        for step in steps {
            ...
            match *step {
                ...
                RequestFrameStep::VarUIntField { field } => {
                    let value = self.use_field(fields, field)?;
                    let (encoded, len) =
                        crate::internal_protocol::encode_varuint(canonical_u64(value)?);
                ...
                | RequestFrameStep::ValueLengthPrefixField { field } => {
                    ...
                    let (encoded, len) =
                        crate::internal_protocol::encode_varuint(value.len() as u64);
                    self.push_encoded(&encoded[..len])?;
    ...
    fn push_encoded(&mut self, value: &[u8]) -> Result<()> {
        let mut bytes = [0; crate::internal_protocol::MAX_VARUINT_BYTES];
...
fn validate_value_length(value_len: usize) -> Result<()> {
    if value_len > crate::internal_protocol::MAX_VALUE_BYTES {
        return Err(ProtocolError::ValueTooLarge {
            size: value_len,
