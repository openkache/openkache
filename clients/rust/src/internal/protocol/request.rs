...
use crate::internal_protocol::{MAX_VALUE_BYTES, OPCODE_BYTES, Opcode, ProtocolError, Result};
...
fn decode_request_frame_metadata_progress<const PROJECT_FIELDS: bool>(
    ...
    let Some((request_id, request_id_len)) = crate::internal_protocol::decode_varuint(
        prefix.get(OPCODE_BYTES..).unwrap_or_default(),
        "request ID",
    )?
    else {
    ...
    for step in steps {
        ...
        match *step {
            ...
            RequestFrameStep::ValueLength => {
                let Some((length, encoded_len)) = crate::internal_protocol::decode_varuint(
            ...
            RequestFrameStep::VarUInt => {
                let Some((_, encoded_len)) = crate::internal_protocol::decode_varuint(
                    prefix.get(state.cursor..).unwrap_or_default(),
                    "request integer",
                )?
                else {
            ...
            RequestFrameStep::VarUIntField { field } => {
                let Some((value, encoded_len)) = crate::internal_protocol::decode_varuint(
                    prefix.get(state.cursor..).unwrap_or_default(),
                    "request integer",
                )?
                else {
