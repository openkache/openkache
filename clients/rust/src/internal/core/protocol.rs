...
use crate::internal_protocol::{NAMESPACE_ID_BYTES, NAMESPACE_REVISION_BYTES, Opcode};

use crate::internal_core::request::RequestRetryPolicy;
...
pub enum ProtocolError {
    ...
    #[error(transparent)]
    Wire(#[from] crate::internal_protocol::ProtocolError),
...
impl NamespaceDescriptor {
    ...
    pub fn decode(input: &[u8]) -> Result<Self> {
        ...
        if input.len() < fixed {
            return Err(crate::internal_protocol::ProtocolError::FrameTooShort {
                expected: fixed,
                actual: input.len(),
            }
            .into());
        ...
        if fixed + policy_len != input.len() {
            return Err(crate::internal_protocol::ProtocolError::FrameLength {
...
fn generated_retry_policy(opcode: Opcode, create_if_missing: bool) -> RequestRetryPolicy {
    use crate::internal_core::contract::OperationRetryMode;

    match crate::internal_core::contract::operation_client_projection(opcode)
        .map(|value| value.retry_mode)
    {
...
fn read_u64_be(input: &[u8]) -> Result<u64> {
    let bytes: [u8; NAMESPACE_ID_BYTES] = input
        ...
        .ok_or(crate::internal_protocol::ProtocolError::FrameTooShort {
