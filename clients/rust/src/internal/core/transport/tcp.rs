...
use crate::internal_core::config::PQ_GROUP;
...
use crate::internal_core::{Error, Operation, Result};
...
impl TlsTcpTransport {
    ...
    pub async fn connect(
        ...
        let server_name =
            rustls::pki_types::ServerName::try_from(server_name.to_owned()).map_err(|error| {
                crate::internal_core::Error::configuration("server_name", error.to_string())
            })?;
        let socket = tokio::time::timeout(timeout, TcpStream::connect(address))
            .await
            .map_err(|_| crate::internal_core::Error::Timeout {
                operation: crate::internal_core::Operation::ConnectionSetup,
            })?
            .map_err(crate::internal_core::Error::io)?;
        socket
            .set_nodelay(true)
            .map_err(crate::internal_core::Error::io)?;
        ...
        let stream = tokio::time::timeout(timeout, connector.connect(server_name, socket))
            .await
            .map_err(|_| crate::internal_core::Error::Timeout {
                operation: crate::internal_core::Operation::Handshake,
            })?
            .map_err(crate::internal_core::Error::io)?;
    ...
    pub(crate) fn negotiated_alpn(&self) -> Option<&[u8]> {
        Some(crate::internal_protocol::ALPN)
    }
}
...
fn validate_handshake(stream: &ClientTlsStream) -> crate::internal_core::Result<()> {
    ...
    if connection.alpn_protocol() != Some(crate::internal_protocol::ALPN) {
        return Err(crate::internal_core::Error::Connection(
...
impl ClientLane for Lane<'_> {
    ...
    async fn read_response(
        ...
        maximum: usize,
        deadline: Deadline,
    ) -> Result<crate::internal_protocol::ResponseParts> {
