...
use crate::internal_protocol::MAX_RESPONSE_FRAME_BYTES;
use crate::internal_protocol::{Response, ResponseHeaderBytes, ResponseParts};
use crossfire::{MAsyncRx, MAsyncTx};
use futures_util::{FutureExt, pin_mut, select};

use crate::internal_core::request::RequestAttempt;
use crate::internal_core::{Backend, Error, Operation, Result};
...
pub(crate) fn enforce_client_profile(
    tls: &mut rustls::ClientConfig,
) -> crate::internal_core::Result<()> {
    if tls.alpn_protocols.len() != 1
        || tls.alpn_protocols[0].as_slice() != crate::internal_protocol::ALPN
    {
    ...
    if groups.len() != 1 || groups[0].name() != crate::internal_core::config::PQ_GROUP {
        return Err(crate::internal_core::Error::configuration(
            "tls.kx_groups",
