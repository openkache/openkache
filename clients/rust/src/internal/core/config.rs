...
use crate::internal_core::protocol::{EvictionMode, ExpirationMode, SetCondition, SetWireOptions};
...
impl AlpnPolicy {
    ...
    pub fn new(protocols: Vec<Vec<u8>>, minimum_version: u32) -> Result<Self> {
        ...
        if protocol.as_slice() != crate::internal_protocol::ALPN {
            return Err(Error::configuration(
                "alpn.protocols",
                format!(
    ...
    pub(crate) fn validate_negotiated(&self, protocol: &[u8]) -> Result<()> {
        ...
        if protocol != crate::internal_protocol::ALPN {
            return Err(Error::Connection(format!(
...
impl Default for AlpnPolicy {
    fn default() -> Self {
        let minimum_version = parse_alpn_version(crate::internal_protocol::ALPN)
            .expect("the generated OpenKache ALPN must contain a positive version");
        Self {
            protocols: vec![crate::internal_protocol::ALPN.to_vec()],
            minimum_version,
        }
    }
}
    ...
    fn default() -> Self {
        Self {
            connect: Duration::from_millis(
                crate::internal_core::contract::DEFAULT_CONNECT_TIMEOUT_MILLISECONDS,
            ),
            request: Duration::from_millis(
                crate::internal_core::contract::DEFAULT_REQUEST_TIMEOUT_MILLISECONDS,
            ),
        }
    }
}
...
impl Default for RetryPolicy {
    fn default() -> Self {
        Self {
            max_attempts: crate::internal_core::contract::DEFAULT_RETRY_MAX_ATTEMPTS,
        }
    }
}
