...
def _item_id_or_pair(value: bytes | bytearray | memoryview) -> bytes:
    item_id = _as_bytes(value, "item_id")
    # A single-ID operation carries the raw ID. Repeated-ID operations carry
    # a length-delimited sequence; validate that sequence when it is present,
    # while leaving malformed/short raw IDs for the native contract to reject.
    offset = 0
    parsed = True
    for _ in range(2):
        if offset >= len(item_id):
            parsed = False
            break
        length = item_id[offset]
        offset += 1
        if length > SMITHY_ITEM_ID_BYTES or length > len(item_id) - offset:
            parsed = False
            break
        offset += length
    if parsed and offset == len(item_id):
        return item_id
    if len(item_id) <= SMITHY_ITEM_ID_BYTES:
        return item_id
    raise OpenKacheValueError("item_id pair contains a truncated or oversized ID")
