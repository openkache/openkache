...
final class Client with SmithyGeneratedOperations implements OpenKacheClient {
  ...
  _NativeResult _invokeScoped(
    ...
    if (requiredItemIdBytes != 0 && itemId.length > requiredItemIdBytes) {
      throw ArgumentError(
        'itemId must contain at most $requiredItemIdBytes bytes',
      );
    }
...
_NativeResult _readResult(
  ...
  try {
    ...
    final status = api.resultStatus(result);
    ...
    return _NativeResult(kind, status, payload, client);
  } finally {
...
final class _NativeResult {
  const _NativeResult(this.kind, this.status, this.payload, this.client);
