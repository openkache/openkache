...
use openkache_protocol::{
    MAX_OPERATION_FIELDS, MAX_OPERATION_REQUEST_FIELDS, OperationFieldLayout, OperationFieldPlan,
    OperationLayoutFraming, OperationLayoutPlan, decode_planned_fields, encode_planned_fields,
    operation_wire_spec, wire_codec_kind,
};
...
pub(crate) fn request_from_contract_body(
    ...
    let contract = operation_wire_spec(operation);
    match contract.request.framing {
        ...
        OperationLayoutFraming::OptionalValues => Err(crate::Error::configuration(
            "operation",
...
/// descriptor.
pub(crate) fn validate_operation_field(field: &OperationFieldPlan, payload: &[u8]) -> Result<()> {
...
pub(crate) fn decode_field_sequence_view<'a>(
    payload: &'a [u8],
    plan: &'static [OperationFieldPlan],
...
pub(crate) fn decode_response_fields_view<'a>(
    ...
    let fields = match plan.framing {
        OperationLayoutFraming::OptionalValues => decode_optional_values_view(payload, plan),
        OperationLayoutFraming::FieldSequence | OperationLayoutFraming::OrderedFields => {
            decode_field_sequence_view(payload, plan.fields, plan.layout)
        }
        OperationLayoutFraming::Empty | OperationLayoutFraming::Opaque => {
