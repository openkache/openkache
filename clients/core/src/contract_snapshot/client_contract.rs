...
pub const CLIENT_GATE0_ALPN_VERSION: u32 = 1;
/// Gate 0's fixed value-compression selector.
pub const CLIENT_GATE0_COMPRESSION: u8 = 0x00;
/// Gate 0's fixed value-encryption selector.
pub const CLIENT_GATE0_ENCRYPTION: u8 = 0x00;
/// Gate 0's fixed public Item-ID root.
pub const CLIENT_GATE0_ITEM_ID_ROOT: [u8; 32] = [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f];
...
pub const fn operation_result_kind(
    ...
    match opcode {
        openkache_protocol::Opcode::Ping => match status {
            openkache_protocol::Status::Ok => Some(FfiResultKind::Ok),
            _ => None,
        },
        openkache_protocol::Opcode::Get => match status {
            openkache_protocol::Status::Ok => Some(FfiResultKind::Value),
            openkache_protocol::Status::NotFound => Some(FfiResultKind::NotFound),
            _ => None,
        },
        openkache_protocol::Opcode::Set => match status {
            openkache_protocol::Status::Created => Some(FfiResultKind::Created),
            openkache_protocol::Status::Replaced => Some(FfiResultKind::Replaced),
            openkache_protocol::Status::NotStored => Some(FfiResultKind::NotStored),
            _ => None,
        },
        openkache_protocol::Opcode::Delete => match status {
            openkache_protocol::Status::Deleted => Some(FfiResultKind::Deleted),
            openkache_protocol::Status::NotFound => Some(FfiResultKind::NotDeleted),
            _ => None,
        },
        openkache_protocol::Opcode::ExperimentalStats => match status {
            openkache_protocol::Status::Ok => Some(FfiResultKind::Value),
            _ => None,
        },
        openkache_protocol::Opcode::ExperimentalSync => match status {
            openkache_protocol::Status::Ok => Some(FfiResultKind::Ok),
            _ => None,
        },
        openkache_protocol::Opcode::NamespaceOpen => match status {
            openkache_protocol::Status::Ok => Some(FfiResultKind::Ok),
            openkache_protocol::Status::Created => Some(FfiResultKind::Created),
            _ => None,
        },
        openkache_protocol::Opcode::NamespaceUpdatePolicy => match status {
            openkache_protocol::Status::Ok => Some(FfiResultKind::Value),
            _ => None,
        },
        openkache_protocol::Opcode::NamespaceDelete => match status {
            openkache_protocol::Status::Deleted => Some(FfiResultKind::Ok),
            _ => None,
        },
    }
}
...
pub const fn protocol_opcode(
    ...
    match operation {
        FfiOperation::Ping => Some(openkache_protocol::Opcode::Ping),
        FfiOperation::Get => Some(openkache_protocol::Opcode::Get),
        FfiOperation::Set => Some(openkache_protocol::Opcode::Set),
        FfiOperation::Delete => Some(openkache_protocol::Opcode::Delete),
        FfiOperation::ExperimentalStats => Some(openkache_protocol::Opcode::ExperimentalStats),
        FfiOperation::ExperimentalSync => Some(openkache_protocol::Opcode::ExperimentalSync),
        FfiOperation::NamespaceOpen => Some(openkache_protocol::Opcode::NamespaceOpen),
        FfiOperation::NamespaceUpdatePolicy => Some(openkache_protocol::Opcode::NamespaceUpdatePolicy),
        FfiOperation::NamespaceDelete => Some(openkache_protocol::Opcode::NamespaceDelete),
        _ => None,
    }
}
