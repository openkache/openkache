...
use super::{
    NamespaceRegistry, NetworkWorkerCache, namespace_exists, operation_codecs,
};
...
///
/// This table is API-owned data layered on the generic extension foundation.
/// The generated opcode index keeps dispatch constant-time, while the codec
/// names are checked against the Smithy descriptor during server startup.
const APPLICATION_VALUE_ENTRIES: &[ApplicationValueExtension] = &[
    ...
    ApplicationValueExtension {
        ...
        request_codec: "packed_f64_be",
        response_codec: "packed_f64_be",
        handler: square_array_application_value,
    },
];

/// The API branch's static application-value behavior table.
pub(super) const APPLICATION_VALUE_EXTENSIONS: ExtensionTable =
    ExtensionTable::with_application_values(APPLICATION_VALUE_ENTRIES);
...
pub(super) fn handles(opcode: Opcode) -> bool {
    opcode == Opcode::Get2 || APPLICATION_VALUE_EXTENSIONS.handles_application_value(opcode)
}
...
fn echo_application_value(value: Vec<u8>) -> Result<Vec<u8>, ExtensionError> {
    operation_codecs::decode_utf8(&value)
        .map_err(ExtensionError::InvalidRequest)?;
    Ok(value)
}

fn reverse_utf8_application_value(value: Vec<u8>) -> Result<Vec<u8>, ExtensionError> {
...
fn square_array_application_value(value: Vec<u8>) -> Result<Vec<u8>, ExtensionError> {
...
async fn execute_get2(
    ...
    let namespace_id = match input.unsigned_long_at_role(
        crate::contract::OperationFieldRole::NamespaceId,
        0,
    ) {
        Ok(Some(namespace_id)) => namespace_id,
        Ok(None) => {
            return Some(ExtensionResponse::invalid_request(
                b"GET2 requires a namespace ID",
            ));
        }
        Err(message) => return Some(ExtensionResponse::invalid_request(message)),
    };
    let item_role = crate::contract::OperationFieldRole::ItemId;
    if input.field_count_role(item_role) != 2 {
        return Some(ExtensionResponse::invalid_request(
            b"GET2 requires exactly two item IDs",
        ));
    }
    let (Some(first_item), Some(second_item)) = (
        input.item_id_at_role(item_role, 0),
        input.item_id_at_role(item_role, 1),
    ) else {
        return Some(ExtensionResponse::invalid_request(
    ...
    if !namespace_exists(namespaces, namespace_id) {
        return Some(ExtensionResponse::Error(
            ExtensionError::NamespaceNotFound,
        ));
    }
        ...
        match result {
            Ok(value) => {
                if value.is_none() {
                    ...
                    if pruned.is_err() {
                        return Some(ExtensionResponse::Error(ExtensionError::Internal(
            ...
            Err(error) => {
                return Some(ExtensionResponse::Error(ExtensionError::Storage(error)));
            }
        }
    }

    Some(ExtensionResponse::field_sequence(values))
}
