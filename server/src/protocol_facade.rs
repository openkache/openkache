...
use super::{
    FrameLayoutProvider, ProtocolError, RequestFrame, Result, ServerRequest, SetOptions, compat_v1,
    generic,
};
use openkache_protocol::{
    ITEM_ID_BYTES, OwnedRange, Opcode, REQUEST_FIXED_BYTES,
};
...
/// it beside the typed request facade prevents compatibility namespace/item
...
///
/// The operation-neutral [`RequestFrame`] exposes only
/// `openkache_protocol::RequestFrameHeader`. This richer header remains in
/// the compatibility facade because its convenience accessors describe the
/// protocol-v1 namespace/item/SET projection.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
