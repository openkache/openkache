...
pub(crate) use storage_keys::{
    DOMAIN_V1_CONTEXT, ITEM_ID_STORAGE_SCOPE, SCOPED_STORAGE_ADDRESS_TAG, derive_domain_key,
...
impl ThreadedKvkache {
    ...
    fn start_with_server_secret(
        ...
        let completion_capacity = queue_capacity
            .checked_add(config.io_uring.retained_response_capacity()?)
            .ok_or_else(|| {
        ...
        for thread_id in 0..config.runtime.thread_count {
            ...
            workers.push(WorkerHandle {
                sender,
                // Cover both requests waiting in the transport queue and requests already
                // retained by the worker. Compact admission never falls back to heap slots.
                completions: CompletionSlab::with_retained_capacity(completion_capacity),
                core_tasks,
                thread: Some(thread),
            });
        }
