...
#[cfg(test)]
/// Encodes explicit mini-bucket counts for independent metadata tests.
fn encode_metadata<const N: usize>(counts: &[usize; MINI_BUCKETS], metadata: &mut [u8; N]) {
    let mut bits = 0u128;
    ...
    debug_assert!(position <= N * 8);
    store_metadata(bits, metadata);
}
...
#[cfg(test)]
mod tests {
    //! Layout, packing, correctness, and SIMD-equivalence tests for BCF53.

    use super::*;
    use crate::Key;
    use std::mem::{align_of, offset_of};

    /// Cache-line-aligned bytes used to verify architecture match masks.
    #[repr(C, align(64))]
    struct AlignedTestBlock(
        /// Complete cache line compared by every backend.
        [u8; 64],
    );

    /// Converts a deterministic numeric test key into its canonical SHA-256 key.
    fn hashed_key(value: u64) -> HashedKey {
        Key::new(value.to_le_bytes().to_vec()).hashed_key()
    }

    /// Verifies the reference implementation's exact 64-byte bucket layouts.
    #[test]
    fn stored_bucket_layout_matches_reference_bcf53() {
        assert_eq!(FRONT_METADATA_BYTES, 13);
        assert_eq!(FRONT_CAPACITY, 51);
        assert_eq!(BACK_METADATA_BYTES, 11);
        assert_eq!(BACK_CAPACITY, 35);
        assert_eq!(CRUMB_BYTES, 18);

        assert_eq!(size_of::<FrontBucket>(), 64);
        assert_eq!(size_of::<BackBucket>(), 64);
        assert_eq!(align_of::<FrontBucket>(), 64);
        assert_eq!(align_of::<BackBucket>(), 64);

        assert_eq!(offset_of!(FrontBucket, metadata), 0);
        assert_eq!(offset_of!(FrontBucket, remainders), 13);
        assert_eq!(offset_of!(BackBucket, metadata), 0);
        assert_eq!(offset_of!(BackBucket, remainders), 11);
        assert_eq!(offset_of!(BackBucket, crumbs), 46);

        assert_eq!(FRONT_METADATA_BYTES + FRONT_CAPACITY, 64);
        assert_eq!(BACK_METADATA_BYTES + BACK_CAPACITY + CRUMB_BYTES, 64);
    }

    /// Verifies 64-bit size, alignment, and field offsets of transient structs.
    #[test]
    fn transient_structs_have_expected_64_bit_abi_layout() {
        assert_eq!(size_of::<usize>(), 8);
        assert_eq!(size_of::<Fingerprint>(), 24);
        assert_eq!(align_of::<Fingerprint>(), 8);
        assert_eq!(offset_of!(Fingerprint, front), 0);
        assert_eq!(offset_of!(Fingerprint, mini), 8);
        assert_eq!(offset_of!(Fingerprint, remainder), 16);

        assert_eq!(size_of::<BackLocation>(), 16);
        assert_eq!(align_of::<BackLocation>(), 8);
        assert_eq!(offset_of!(BackLocation, bucket), 0);
        assert_eq!(offset_of!(BackLocation, crumb), 8);

        assert_eq!(size_of::<Overflow>(), 16);
        assert_eq!(align_of::<Overflow>(), 8);
        assert_eq!(offset_of!(Overflow, mini), 0);
        assert_eq!(offset_of!(Overflow, remainder), 8);
    }

    /// Verifies that an empty bucket stores exactly 53 contiguous separators.
    #[test]
    fn empty_metadata_has_exact_reference_bit_pattern() {
        let expected_prefix = [0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x1f];
        let front = FrontBucket::default();
        let back = BackBucket::default();

        assert_eq!(&front.metadata[..7], &expected_prefix);
        assert!(front.metadata[7..].iter().all(|&byte| byte == 0));
        assert_eq!(&back.metadata[..7], &expected_prefix);
        assert!(back.metadata[7..].iter().all(|&byte| byte == 0));
        assert_eq!(metadata_value(&front.metadata).count_ones(), 53);
        assert_eq!(metadata_value(&back.metadata).count_ones(), 53);
        assert_eq!(front.len(), 0);
        assert_eq!(back.len(), 0);
    }

    /// Verifies unary separator counts and bounds across insertion and deletion.
    #[test]
    fn unary_metadata_stays_packed_after_mutations() {
        let mut bucket = FrontBucket::default();
        let mut expected_counts = [0usize; MINI_BUCKETS];
        let entries = [(52, 1), (0, 2), (7, 3), (7, 4), (31, 5), (0, 6)];

        for (inserted, &(mini, remainder)) in entries.iter().enumerate() {
            assert!(bucket.insert(mini, remainder).is_none());
            expected_counts[mini] += 1;

            let metadata = metadata_value(&bucket.metadata);
            assert_eq!(metadata.count_ones(), MINI_BUCKETS as u32);
            assert_eq!(bucket.len(), inserted + 1);
            assert_eq!(metadata_counts(&bucket.metadata), expected_counts);
            assert_eq!(metadata >> (MINI_BUCKETS + inserted + 1), 0);
        }

        assert!(bucket.remove(7, 4, SimdBackend::Scalar));
        expected_counts[7] -= 1;
        assert_eq!(metadata_counts(&bucket.metadata), expected_counts);
        assert_eq!(metadata_value(&bucket.metadata).count_ones(), 53);
        assert_eq!(bucket.len(), entries.len() - 1);
    }

    /// Verifies two four-bit crumbs share each byte and remain packed after removal.
    #[test]
    fn backyard_crumbs_are_packed_as_two_nibbles_per_byte() {
        let mut bucket = BackBucket::default();
        bucket.insert(10, 1, 1).unwrap();
        bucket.insert(3, 2, 2).unwrap();
        bucket.insert(10, 3, 3).unwrap();

        assert_eq!(&bucket.remainders[..3], &[2, 3, 1]);
        assert_eq!(bucket.crumbs[0], 0x32);
        assert_eq!(bucket.crumbs[1], 0x01);
        assert_eq!(bucket.crumb(0), 2);
        assert_eq!(bucket.crumb(1), 3);
        assert_eq!(bucket.crumb(2), 1);
        assert!(bucket.crumbs[2..].iter().all(|&byte| byte == 0));

        assert!(bucket.remove(10, 3, 3, SimdBackend::Scalar));
        assert_eq!(&bucket.remainders[..2], &[2, 1]);
        assert_eq!(bucket.crumbs[0], 0x12);
        assert!(bucket.crumbs[1..].iter().all(|&byte| byte == 0));
    }

    /// Verifies explicit mini-bucket counts survive metadata encode/decode.
    #[test]
    fn metadata_round_trip() {
        let mut metadata = [0; FRONT_METADATA_BYTES];
        let mut counts = [0usize; MINI_BUCKETS];
        counts[0] = 2;
        counts[7] = 3;
        counts[52] = 1;
        encode_metadata(&counts, &mut metadata);
        assert_eq!(metadata_len(&metadata), 6);
        assert_eq!(metadata_counts(&metadata), counts);
        assert_eq!(metadata_bounds(&metadata, 0), (0, 2));
        assert_eq!(metadata_bounds(&metadata, 7), (2, 5));
        assert_eq!(metadata_bounds(&metadata, 52), (5, 6));
    }

    /// Verifies fingerprinting uses hash bytes directly without another hash pass.
    #[test]
    fn fingerprint_uses_hashed_key_bytes_directly() {
        let filter = BreadcrumbFilter::with_capacity(1_000);
        let hash_prefix = 0x0123_4567_89ab_cdefu64;
        let mut bytes = [0u8; 32];
        bytes[..8].copy_from_slice(&hash_prefix.to_le_bytes());
        let hashed_key = HashedKey::new(bytes);

        let fingerprint_space = (filter.front.len() * MINI_BUCKETS) as u64 * 256;
        let expected = hash_prefix % fingerprint_space;
        let actual = filter.fingerprint(&hashed_key);

        assert_eq!(actual.front, (expected >> 8) as usize / MINI_BUCKETS);
        assert_eq!(actual.mini, (expected >> 8) as usize % MINI_BUCKETS);
        assert_eq!(actual.remainder, expected as u8);
    }

    /// Verifies insertion, promotion-aware deletion, reinsertion, and no false negatives.
    #[test]
    fn insert_query_delete_without_false_negatives() {
        let count = 100_000usize;
        let mut filter = BreadcrumbFilter::with_capacity(count);
        let keys: Vec<_> = (0..count as u64).map(|key| hashed_key(key + 17)).collect();
        for key in &keys {
        ...
        for key in &keys[..count / 2] {
            assert!(filter.remove(key));
        }
        assert_eq!(filter.len(), count / 2);
        assert!(keys[count / 2..].iter().all(|key| filter.contains(key)));
    ...
    /// Verifies runtime SIMD queries return the same result as scalar queries.
    #[test]
    fn simd_and_scalar_queries_agree() {
        let count = 20_000usize;
        let mut filter = BreadcrumbFilter::with_capacity(count);
        for key in 0..count as u64 {
            filter.insert(&hashed_key(key)).unwrap();
        }
        for key in 0..count as u64 * 2 {
            let key = hashed_key(key);
            let fingerprint = filter.fingerprint(&key);
            filter.backend = SimdBackend::Scalar;
            let scalar = filter.contains_fingerprint(fingerprint);
            filter.backend = SimdBackend::detect();
            assert_eq!(filter.contains_fingerprint(fingerprint), scalar);
        }
    }

    /// Verifies the selected byte-comparison backend returns every scalar bit.
    #[test]
    fn selected_backend_matches_all_scalar_byte_masks() {
        let mut block = AlignedTestBlock([0; 64]);
        for (index, byte) in block.0.iter_mut().enumerate() {
            *byte = (index as u8).wrapping_mul(37).wrapping_add(11);
        }

        let pointer = block.0.as_ptr();
        let backend = SimdBackend::detect();
        for needle in u8::MIN..=u8::MAX {
            let scalar = unsafe { byte_matches_scalar(pointer, needle) };
            assert_eq!(byte_matches(&block, needle, backend), scalar);
        }
    }

    /// Verifies the selected rank implementation agrees with scalar selection.
    #[test]
    fn selected_backend_matches_scalar_bit_selection() {
        let patterns = [
            0x5555_aaaa_f0f0_0f0f_1234_5678_9abc_def0u128,
            0x8000_0000_0000_0001_ffff_0000_aaaa_5555u128,
        ];

        for bits in patterns {
            for rank in 0..bits.count_ones() as usize {
                assert_eq!(select_one(bits, rank), select_one_scalar(bits, rank));
            }
        }
    }

    /// Verifies the measured false-positive rate remains below one percent.
    #[test]
    fn measured_false_positive_rate_is_reasonable() {
        ...
        let false_positives = (count as u64..count as u64 * 2)
            .map(hashed_key)
            .filter(|key| filter.contains(key))
            .count();
        assert!(false_positives < count / 100, "{false_positives}");
    }
}
