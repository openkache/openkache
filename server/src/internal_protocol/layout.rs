...
pub(crate) const OPTIONAL_VALUE_CODEC: crate::internal_protocol::OptionalValueCodec =
    match crate::internal_protocol::OptionalValueCodec::new(
        crate::internal_protocol::OPTIONAL_VALUE_LENGTH_BYTES,
        crate::internal_protocol::OPTIONAL_VALUE_MISSING as u64,
    ) {
        Ok(codec) => codec,
...
pub fn encode_dense_fields(values: &[&[u8]], widths: &[usize]) -> Result<Vec<u8>> {
    ...
    crate::internal_protocol::validate_value_length(capacity)?;
...
pub fn encode_layout_fields(
    ...
    match layout {
        ...
        OperationFieldLayout::OptionalValues => {
            crate::internal_protocol::encode_optional_values(OPTIONAL_VALUE_CODEC, values)
        }
...
pub fn decode_layout_fields(
    ...
    match layout {
        OperationFieldLayout::Sequence => {
            crate::internal_protocol::FieldSequence::decode_with_required(
                payload, required, offsets,
            )
            .map(|_| ())
        }
...
pub fn encode_planned_field_segments<I, T>(
    ...
    plan: &[OperationFieldPlan],
    layout: OperationFieldLayout,
    optional_codec: Option<&crate::internal_protocol::OptionalValueCodec>,
) -> Result<OwnedFrame>
where
    ...
    T: Into<WireSegment>,
{
    encode_planned_field_segments_in::<
        [WireSegment; crate::internal_protocol::segments::INLINE_SEGMENTS],
        _,
        _,
...
pub(crate) fn encode_planned_field_segments_in<A, I, T>(
    ...
    layout: OperationFieldLayout,
    optional_codec: Option<&crate::internal_protocol::OptionalValueCodec>,
) -> Result<crate::internal_protocol::SegmentFrame<A>>
where
    A: Array<Item = WireSegment>,
    I: IntoIterator<Item = Option<T>>,
    T: Into<WireSegment>,
{
    if plan.len() > crate::internal_protocol::MAX_OPERATION_FIELDS {
        return Err(ProtocolError::InvalidFieldSequence(
        ...
        OperationFieldLayout::Sequence => {
            ...
            for (index, value) in values.into_iter().enumerate() {
                ...
                if Some(index) != final_present {
                    ...
                    let (encoded, encoded_len) = crate::internal_protocol::encode_varuint(length);
    ...
    let frame = crate::internal_protocol::SegmentFrame::from_segments(segments)?;
    if frame.len() > crate::internal_protocol::MAX_VALUE_BYTES {
        return Err(ProtocolError::ValueTooLarge {
            size: frame.len(),
            maximum: crate::internal_protocol::MAX_VALUE_BYTES,
        });
    }
    Ok(frame)
}
