//! Legacy storage engine: hash table + segment groups + io_uring-backed flush to SSD.
//!
...
use crate::storage_message::{
    Command, Reply, STORAGE_QUEUE_SLOTS, StorageKey, StorageRequest, StorageResponse,
};
...
async fn execute_request(
    ...
    let reply = match command {
        Command::Get { key } => execute_get(worker, file, key).await?,
        Command::Set { key, value } => execute_set(worker, file, key, value).await?,
        Command::Delete { key } => execute_delete(worker, file, key).await?,
