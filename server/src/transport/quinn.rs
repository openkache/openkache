...
impl Endpoint {
    pub(super) async fn bind(
        ...
        let tls = rustls_config(&material)
...
impl super::RequestByteStream for ReceiveStream {
    async fn read_chunk(
        ...
        self.0
            .read_chunk(capacity.max(1), true)
            .await
            .map(|chunk| chunk.map(|chunk| OwnedRange::whole_bytes(chunk.bytes)))
...
impl super::SendStream for SendStream {
    async fn write_response(
        ...
        let result = network_runtime::timeout(timeout, async {
            write_response_segments(&mut self.0, parts)
                .await
