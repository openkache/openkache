...
impl Endpoint {
    pub(super) async fn bind(
        ...
        transport
            ...
            .max_concurrent_uni_streams(compio_quic::VarInt::from_u32(max_concurrent_streams));
...
impl super::Connection for Connection {
    ...
    fn close(&self, error_code: u64, reason: &[u8]) {
        let code = compio_quic::VarInt::from_u64(error_code)
            .unwrap_or_else(|_| compio_quic::VarInt::from_u32(1));
...
impl super::RequestByteStream for ReceiveStream {
    async fn append_chunk(
        ...
        match self.0.read_chunk(capacity, true).await {
            Ok(Some(chunk)) => {
                frame.try_reserve(chunk.bytes.len()).map_err(|error| {
            ...
            Err(compio_quic::ReadError::Reset(_)) => Ok(super::ChunkRead::Cancelled),
...
impl super::ReceiveStream for ReceiveStream {
    ...
    fn stop(&mut self) {
        let _ = self.0.stop(compio_quic::VarInt::from_u32(0));
    }
}
...
impl super::SendStream for SendStream {
    ...
    fn reset(&mut self) {
        let _ = self.0.reset(compio_quic::VarInt::from_u32(0));
    }
}
