...
mod tests {
    ...
    #[test]
    fn defaults_derive_expected_geometry() {
        let derived = Config::default().validate().unwrap();
        assert_eq!(derived.buckets_per_sg, 4_096);
        assert_eq!(derived.sg_bytes, 16 * MIB);
        assert_eq!(derived.storage_file_bytes, 16 * 16 * MIB);
        // 16 SGs -> 4 index bits + 2 bucket-choice bits = 6.
        assert_eq!(derived.table_value_bits, 6);
    }

    #[test]
    fn legacy_geometry_still_derives_eight_value_bits() {
        let mut config = Config::default();
        config.storage_sg_count = 64;
        // 64 SGs -> 6 index bits + 2 bucket-choice bits = 8, the old constant.
        assert_eq!(config.validate().unwrap().table_value_bits, 8);
    }

    #[test]
    fn index_bits_matches_ceil_log2() {
        assert_eq!(index_bits(64), 6);
        assert_eq!(index_bits(65), 7);
        assert_eq!(index_bits(1024), 10);
        assert_eq!(index_bits(1), 1);
    }

    #[test]
    fn rejects_unaligned_sg_size() {
        let mut config = Config::default();
        // 1 MiB is a multiple of 4 KiB; use a byte-level check via a fractional
        // value that is not, by shrinking the Bucket alignment expectation.
        config.sg_size_mib = 0;
        assert!(config.validate().is_err());
    }

    #[test]
    fn rejects_too_few_sgs() {
        let mut config = Config::default();
        config.storage_sg_count = MUTABLE_SG_COUNT + 1;
        assert!(config.validate().is_err());
    }

    #[test]
    fn rejects_sg_count_overflowing_table_value() {
        let mut config = Config::default();
        // Needs index_bits + 2 > 32, i.e. index_bits > 30.
        config.storage_sg_count = 1usize << 31;
        assert!(config.validate().is_err());
    }
}
