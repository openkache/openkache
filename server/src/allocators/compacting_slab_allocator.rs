...
impl<T> Slab<T> {
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        Self::try_with_capacity(capacity)
            .expect("Slab::with_capacity failed: out of memory")
    }
    ...
    pub fn try_insert_handle(
        ...
        unsafe {
            ...
            match self.try_insert(value, &mut *raw) {
                Ok(_pin) => Ok(Pin::new_unchecked(Box::from_raw(
                    raw as *mut Handle<'_, T>,
                ))),
                Err(e) => {
...
mod tests {
    ...
    fn test_insert_and_access() {
        ...
        let mut h = MaybeUninit::<Handle<i32>>::uninit();
        {
            let h_pin = slab.insert(42, &mut h);
            assert_eq!(**h_pin, 42);
            assert_eq!(h_pin.header.index.get(), 0);
            assert_eq!(slab.len(), 1);
        }

        unsafe {
            ptr::drop_in_place(h.as_mut_ptr());
        }
        assert_eq!(slab.len(), 0);
    }

    #[test]
    fn test_swap_pop_middle() {
        let slab = Slab::<i32>::new();
        let mut h1 = MaybeUninit::<Handle<i32>>::uninit();
        let mut h2 = MaybeUninit::<Handle<i32>>::uninit();
        let mut h3 = MaybeUninit::<Handle<i32>>::uninit();

        slab.insert(10, &mut h1);
        slab.insert(20, &mut h2);
        slab.insert(30, &mut h3);

        assert_eq!(slab.len(), 3);

        unsafe {
            // Drop middle handle (h2) → last element (30) fills the hole.
            ptr::drop_in_place(h2.as_mut_ptr());

            assert_eq!(slab.len(), 2);
            assert_eq!(**h1.assume_init_ref(), 10);
            assert_eq!(h1.assume_init_ref().header.index.get(), 0);
            assert_eq!(**h3.assume_init_ref(), 30);
            assert_eq!(h3.assume_init_ref().header.index.get(), 1);

            // Drop h1 → h3's data (30) fills the hole at index 0.
            ptr::drop_in_place(h1.as_mut_ptr());
            assert_eq!(slab.len(), 1);
            assert_eq!(**h3.assume_init_ref(), 30);
            assert_eq!(h3.assume_init_ref().header.index.get(), 0);

            ptr::drop_in_place(h3.as_mut_ptr());
            assert_eq!(slab.len(), 0);
        }
    }

    #[test]
    fn test_swap_pop_last() {
        let slab = Slab::<i32>::new();
        let mut h1 = MaybeUninit::<Handle<i32>>::uninit();
        let mut h2 = MaybeUninit::<Handle<i32>>::uninit();
    ...
    fn test_relocation() {
        let slab = Slab::<i32>::new();
        let mut h1_storage = MaybeUninit::<Handle<i32>>::uninit();
        let mut h1_pin = slab.insert(100, &mut h1_storage);

        assert_eq!(**h1_pin, 100);

        unsafe {
            let mut h2_storage = MaybeUninit::<Handle<i32>>::uninit();
            h1_pin.as_mut().relocate_to(&mut h2_storage);

            let h2_ref = h2_storage.assume_init_ref();
            assert_eq!(**h2_ref, 100);
            assert_eq!(h2_ref.header.index.get(), 0);

            // Verify slot back-pointer was updated.
            {
                let slots = slab.slots_ptr();
                assert_eq!(
                    (*slots).handle_header.as_ptr(),
                    &h2_ref.header as *const _ as *mut _
                );
            }

            ptr::drop_in_place(h2_storage.as_mut_ptr());
            assert_eq!(slab.len(), 0);
        }
    }

    #[test]
    fn test_handle_count_tracking() {
        let slab = Slab::<i32>::new();
        assert_eq!(slab.len(), 0);
        ...
        slab.insert(1, &mut h1);
        assert_eq!(slab.len(), 1);
        slab.insert(2, &mut h2);
        assert_eq!(slab.len(), 2);
    ...
    // NOTE: test_slab_dropped_before_handle_panics is removed because it now
    // fails to compile, which is the desired behavior for lifetime safety.

    #[test]
    fn test_structural_invariant_explicit() {
        let slab = Slab::<i32>::new();
        let mut h1 = MaybeUninit::<Handle<i32>>::uninit();
        let mut h2 = MaybeUninit::<Handle<i32>>::uninit();
        let mut h3 = MaybeUninit::<Handle<i32>>::uninit();
        ...
        // Verify invariant: slots[i].handle_header.get() == i
        unsafe {
            let slots = slab.slots_ptr();
            for i in 0..slab.len() {
                let handle_index = (*slots.add(i)).handle_header.as_ref().index.get();
                assert_eq!(handle_index, i);
            }

            // After deleting middle, invariant still holds.
            ptr::drop_in_place(h2.as_mut_ptr());
            let slots = slab.slots_ptr();
            for i in 0..slab.len() {
                let handle_index = (*slots.add(i)).handle_header.as_ref().index.get();
    ...
    #[test]
    fn test_stress_many_insertions_and_deletions() {
        let slab = Slab::<usize>::new();
        const N: usize = 100;
        let mut handles: Vec<MaybeUninit<Handle<usize>>> =
            (0..N).map(|_| MaybeUninit::uninit()).collect();

        for i in 0..N {
            slab.insert(i, &mut handles[i]);
        }
        assert_eq!(slab.len(), N);

        // Delete every other handle (even indices).
        unsafe {
            for i in (0..N).step_by(2) {
                ptr::drop_in_place(handles[i].as_mut_ptr());
            }
        }
        assert_eq!(slab.len(), N / 2);

        // All remaining handles should still dereference correctly.
        unsafe {
            for i in (1..N).step_by(2) {
                let h = handles[i].assume_init_ref();
                assert_eq!(**h, i);
            }

            // Verify structural invariant holds after bulk deletion.
            let slots = slab.slots_ptr();
            for i in 0..slab.len() {
                let handle_index = (*slots.add(i)).handle_header.as_ref().index.get();
                assert_eq!(handle_index, i);
    ...
    #[test]
    fn test_ergonomic_apis() {
        let slab = Slab::<i32>::new();
        let mut h = MaybeUninit::<Handle<i32>>::uninit();
        {
            let mut h_pin = slab.insert(42, &mut h);
            assert_eq!(**h_pin, 42);

            // test get_mut
            unsafe {
                let h_pin_mut = h_pin.as_mut();
                *Handle::get_mut(h_pin_mut.get_unchecked_mut()) = 43;
            }
            assert_eq!(**h_pin, 43);
        }
    ...
    fn test_geometric_growth() {
        ...
        let slot_size = std::mem::size_of::<Slot<i32>>();
        let slots_per_page = page_size / slot_size;

        // Initial capacity is 0.
        assert_eq!(slab.cap.get(), 0);

        // Reserve 1. Should commit 1 page (slots_per_page capacity).
        slab.reserve(1);
        assert_eq!(slab.cap.get(), slots_per_page);

        // Reserve more than current capacity. Should double current capacity.
        // Current: slots_per_page. Target: slots_per_page + 1.
        // required = 0 + (slots_per_page + 1) = slots_per_page + 1.
        // target_slots = max(required, current * 2) = slots_per_page * 2.
        slab.reserve(slots_per_page + 1);
        assert_eq!(slab.cap.get(), slots_per_page * 2);
    }

    #[test]
    fn test_double_delete_safety() {
        let slab = Slab::<i32>::new();
        let mut h = MaybeUninit::<Handle<i32>>::uninit();
        slab.insert(10, &mut h);

        unsafe {
            let h_ptr = h.as_mut_ptr();
            // First drop
            ptr::drop_in_place(h_ptr);
            assert_eq!(slab.len(), 0);

            // Second drop (should be no-op due to tombstone)
            ptr::drop_in_place(h_ptr);
            assert_eq!(slab.len(), 0);
        }
    }
    ...
    #[test]
    fn test_panic_safety_in_delete() {
        ...
        let slab = Slab::<PanicOnDrop>::new();
        let mut h1 = MaybeUninit::<Handle<PanicOnDrop>>::uninit();
        let mut h2 = MaybeUninit::<Handle<PanicOnDrop>>::uninit();

        slab.insert(PanicOnDrop(true), &mut h1); // This one will panic on delete
        slab.insert(PanicOnDrop(false), &mut h2);

        // We use catch_unwind to simulate panic during delete
        let h1_ptr = h1.as_mut_ptr();
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| unsafe {
            ptr::drop_in_place(h1_ptr);
        }));

        assert!(result.is_err());
        // verify len was decremented (or at least state is consistent)
        // Note: len is decremented before the actual drop that panics.
        assert_eq!(slab.len(), 1);
        assert_eq!(DROP_COUNT.load(Ordering::SeqCst), 1);

        // Clean up second handle to avoid Slab::drop panic
        unsafe {
            ptr::drop_in_place(h2.as_mut_ptr());
        }
        assert_eq!(slab.len(), 0);
    ...
    #[test]
    fn test_non_copy_type_move() {
        // String is !Copy. This test verifies that delete uses move semantics (read/write)
        // rather than bitwise copy, which would cause double-drops.
        let slab = Slab::<String>::new();
        let mut h1 = MaybeUninit::<Handle<String>>::uninit();
        let mut h2 = MaybeUninit::<Handle<String>>::uninit();

        slab.insert("hello".to_string(), &mut h1);
        slab.insert("world".to_string(), &mut h2);

        assert_eq!(slab.len(), 2);

        // Delete h1. This triggers a swap_remove where the String "world"
        // is moved from index 1 to index 0.
        unsafe {
            ptr::drop_in_place(h1.as_mut_ptr());
        }
        ...
        assert_eq!(slab.len(), 1);
        unsafe {
            let h2_ref = h2.assume_init_ref();
            assert_eq!(**h2_ref, "world");
        }

        // Drop slab — if "world" was double-freed (bitwise-copied),
        // this would crash or show up under Miri.
        // String drop is tested here implicitly.
        unsafe {
            ptr::drop_in_place(h2.as_mut_ptr());
        }
        assert_eq!(slab.len(), 0);
    ...
    #[test]
    fn test_insert_handle() {
        let slab = Slab::<i32>::new();
        let h = slab.insert_handle(100);
        assert_eq!(**h, 100);
        assert!(h.is_alive());
        drop(h);
        assert_eq!(slab.len(), 0);
    ...
    fn test_insert_into() {
        let slab = Slab::<i32>::new();
        let mut storage = HandleStorage::uninit();
        let h_pin = slab.insert_into(42, &mut storage);
        assert_eq!(**h_pin, 42);
        assert_eq!(slab.len(), 1);
    ...
    fn test_retain() {
        let slab = Slab::<i32>::new();
        let _h1 = slab.insert_handle(1);
        let _h2 = slab.insert_handle(2);
        let _h3 = slab.insert_handle(3);
        let _h4 = slab.insert_handle(4);
        ...
        assert_eq!(slab.len(), 2);
        let values: Vec<_> = slab.iter().cloned().collect();
    ...
    #[test]
    fn test_stale_handle_safety() {
        let slab = Slab::<i32>::new();
        let mut h1_storage = MaybeUninit::uninit();
        slab.insert(10, &mut h1_storage);

        // Delete h1
        unsafe {
            ptr::drop_in_place(h1_storage.as_mut_ptr());
        }
        assert_eq!(slab.len(), 0);

        // Insert h2 - it should reuse slot 0
        let mut h2_storage = MaybeUninit::uninit();
        let h2_pin = slab.insert(20, &mut h2_storage);

        assert_eq!(h2_pin.index(), 0);

        // Stale handle check (h1 storage still exists)
        // when h1 was deleted, its index cell was set to DEAD.
        // So h1.is_alive() is false because of DEAD.
        unsafe {
            let h1 = h1_storage.assume_init_ref();
            assert!(!h1.is_alive());

            // Let's bypass the DEAD check to test structural identity.
            h1.header.index.set(0); // Force index 0 (matching h2)

            // This should still fail is_alive because pointer mismatch
            // (h1 header address != h2 header address stored in slot)
            assert!(!h1.is_alive());

            // If debug assertions are on, deref should panic!
    ...
    fn test_iterator_epoch_safety() {
        ...
        let mut iter = slab.iter();
        iter.next(); // OK, first element

        // Mutate slab while iterator still has one element left.
        let mut h3 = MaybeUninit::<Handle<i32>>::uninit();
        slab.insert(3, &mut h3);

        // Catch the expected panic from iter.next()
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(move || {
            let _ = iter.next();
        }));

        // Clean up handles so Slab::drop doesn't panic.
        unsafe {
            ptr::drop_in_place(h1.as_mut_ptr());
            ptr::drop_in_place(h2.as_mut_ptr());
            ptr::drop_in_place(h3.as_mut_ptr());
        ...
        let msg = result
            .err()
            .and_then(|any| any.downcast_ref::<String>().cloned())
            .unwrap_or_default();
        assert!(msg.contains("Slab mutated during iteration — iterator invalidated!"));
    }
    ...
    fn test_deref_mut() {
        ...
        let mut h = MaybeUninit::<Handle<i32>>::uninit();
        slab.insert(10, &mut h);
        let mut h_pin = unsafe { Pin::new_unchecked(h.assume_init_mut()) };

        // Test DerefMut
        unsafe {
            let h_val_mut = h_pin.as_mut().get_unchecked_mut();
            **h_val_mut = 20;
        }
        assert_eq!(**h_pin, 20);
    ...
    fn test_zst() {
        let slab = Slab::<()>::new();
        let mut h1 = MaybeUninit::<Handle<()>>::uninit();
        let mut h2 = MaybeUninit::<Handle<()>>::uninit();

        slab.insert((), &mut h1);
        slab.insert((), &mut h2);

        assert_eq!(slab.len(), 2);
        assert_eq!(slab.contains(unsafe { h1.assume_init_ref() }), true);
    ...
    #[test]
    fn test_structural_identity_collision() {
        let slab = Slab::<i32>::new();
        let mut h1_storage = MaybeUninit::uninit();
        slab.insert(10, &mut h1_storage);

        // Force a collision scenario:
        // 1. Delete h1. Slot 0 is free.
        unsafe {
            ptr::drop_in_place(h1_storage.as_mut_ptr());
        }

        // 2. Insert h2. It takes slot 0.
        let mut h2_storage = MaybeUninit::uninit();
        slab.insert(20, &mut h2_storage);

        // 3. Delete h2. Slot 0 is free again.
        unsafe {
            ptr::drop_in_place(h2_storage.as_mut_ptr());
        }

        // 4. Manually manipulate h1 to point to its old slot.
        // In a real usage, a stale handle might have been copied/stored somewhere.
        unsafe {
            let h1 = h1_storage.assume_init_ref();
            h1.header.index.set(0);

            // Header pointer mismatch check:
            // &h1.header != slot.handle_header (which is null or pointed to some dead handle's stack etc)
            assert!(!h1.is_alive());
        }
    }

    #[test]
    fn test_slab_shrink_to_fit() {
        let slab = Slab::<i32>::new();

        // 1. Fill slab to force growth.
        let page_size = unsafe { (*slab.stack.get()).page_size() };
        let slot_size = std::mem::size_of::<Slot<i32>>();
        let slots_per_page = page_size / slot_size;

        let items_to_fill = slots_per_page * 2;
        let mut handles = Vec::new();
        for i in 0..items_to_fill {
            handles.push(slab.insert_handle(i as i32));
        }

        let cap_before = slab.cap.get();
        assert!(cap_before >= items_to_fill);

        // 2. Clear most items.
        handles.truncate(1); // Keep only the first one.
        assert_eq!(slab.len(), 1);

        // 3. Shrink to fit.
        let old_epoch = slab.epoch.get();
        slab.shrink_to_fit();
        let cap_after = slab.cap.get();

        assert!(cap_after < cap_before);
        assert!(cap_after >= 1);
        assert_eq!(slab.len(), 1);
        assert_ne!(slab.epoch.get(), old_epoch); // shrink_to_fit bumps epoch to invalidate iterators

        // 4. Verify remaining item is still valid.
        assert_eq!(**handles[0], 0);
        assert!(handles[0].is_alive());

        // 5. Verify re-growth works.
        for i in 0..items_to_fill {
            handles.push(slab.insert_handle(i as i32));
        }
        assert!(slab.cap.get() >= items_to_fill + 1);
    }
}
