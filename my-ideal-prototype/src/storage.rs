...
impl Storage {
    ...
    fn commit_set(
        ...
        let mut flush = None;
        let replacement = match self.append_to_mutable_sg(
            key,
            BucketValue::Value(value),
        ...
        let table_updated = match observation.previous {
            Some(previous) => self
                .table
                .replace(key.table_hash(), previous, replacement),
...
async fn execute_set(
    ...
    loop {
        ...
        let observation =
            resolve_set_plan(worker, Rc::clone(&file), &key, &plan).await?;
        let commit = worker.access(|worker| {
            worker
                .storage
        ...
        match commit {
            ...
            CommitSet::Finished { result, flush } => {
                ...
                result.map_err(|error| {
                    io::Error::other(format!("storage SET failed: {error:?}"))
                })?;
            ...
            runtime
                .spawn(async move {
                    match execute_request(worker_handle, file, request).await {
                        Ok(response) => worker_handle
                            .access(|worker| worker.completed.push_back(response)),
                        Err(error) => {
