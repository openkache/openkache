...
**OpenKache는 왜 빠른가? 요청이 처음 도착한 코어를 떠나지 않기 때문이다.**

대부분의 서버는 코어 사이를 떠도는 스레드 풀에 일을 흩뿌립니다. 그 대가는 공짜가 아닙니다.
요청이 코어를 건널 때마다 값을 치릅니다. 락 경합, 뮤텍스, 컨텍스트 스위치, 그리고 캐시 사이를
튕겨 다니는 캐시 라인, 그 하나하나가 동기화와 복사를 강제합니다. 부하가 걸릴수록 처리량의
천장을 정하는 건 실제 작업이 아니라 이 조율 비용입니다.

OpenKache는 그 값을 치르길 거부합니다. 아래의 각 결정은 그 비용을 통째로 지웁니다:

| 설계 결정 | 지워버리는 비용 | 근거 |
|---|---|---|
| **Thread-per-core, shared-nothing** | 락 경합, 컨텍스트 스위치, 코어 간 캐시 라인 바운싱 | [KVell (SOSP '19)](https://dl.acm.org/doi/10.1145/3341301.3359628) |
| **세그먼트 그룹 순차 쓰기** | 랜덤 쓰기 IOPS, 플래시 write amplification | [FairyWren (OSDI '24)](https://www.usenix.org/conference/osdi24/presentation/mcallister) |
| **`io_uring` + 다이렉트 I/O** | 시스템 콜 오버헤드, 커널 페이지 캐시 복사 | — |
| **Rust로 작성** | GC 일시정지, 프로덕션이 아니라 컴파일러가 잡는 데이터 레이스 | — |

KVell의 교훈은 직설적입니다: 최신 SSD에서 병목은 더 이상 디스크가 아닙니다. CPU입니다. 그래서
OpenKache는 각 워커를 자기 코어에 고정하고, 자기 데이터만 쥐여주고, 락 하나 없이 달리게 합니다.
네트워크 워커와 스토리지 워커는 각각 코어를 소유하고 하나의 락 프리 SPSC 큐로 대화하므로, 파싱이
디스크 I/O를 막지 않고 디스크 I/O도 파싱을 막지 않습니다.

FairyWren은 두 번째를 짚습니다: 플래시 캐시는 작은 랜덤 쓰기로 수명을 태웁니다. 그래서
OpenKache는 그런 쓰기를 아예 내지 않습니다. 여러 키를 하나의 순차 **세그먼트 그룹** flush로
묶어, 승객마다 차 한 대가 아니라 만원 열차 한 편으로, SSD의 순차 대역폭을 천장까지 씁니다.

**Rust이기에 한계까지 밀어붙여도 안전합니다. GC가 빠른 경로를 얼릴 일이 없고, 데이터 레이스는
프로덕션이 아니라 컴파일러가 잡습니다.**

전체 설계는 [docs/architecture.ko.md](./docs/architecture.ko.md) 참고.
